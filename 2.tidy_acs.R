...
# 2014 ACS block Level Data (for race): http://old.socialexplorer.com/pub/reportdata/CsvResults.aspx?reportid=R11210348
...
d15 <- read.csv("./data/R11382097_SL140.csv",stringsAsFactors = FALSE,skip = 1) %>%
  select(Geo_FIPS,Geo_NAME,Geo_STATE,Geo_COUNTY,Geo_TRACT,
         kid1 = SE_T007_004,
         kid2 = SE_T007_005,
         pov18=SE_T114_002,
...
# 2015 ACS block Level Data (for race): http://old.socialexplorer.com/pub/reportdata/CsvResults.aspx?reportid=R11210348
df15 <- read.csv("./data/R11382087_SL150.csv",stringsAsFactors = FALSE,skip = 1) %>%
  select(Geo_FIPS,Geo_NAME,Geo_STATE,Geo_COUNTY,Geo_TRACT,Geo_BLKGRP,
         pop = SE_T013_001,
         white_alone = ACS15_5yr_B02001002,
         pacific_alone = ACS15_5yr_B02001006,
         black=ACS15_5yr_B02009001,
         indian=ACS15_5yr_B02010001,
         hisp=SE_T014_010,
         asian=ACS15_5yr_B02011001) %>% 
  mutate(asian=asian+pacific_alone,
         fips11=ifelse(nchar(Geo_FIPS)>11,Geo_FIPS,paste(0,Geo_FIPS,sep="")),
         fips11=substring(fips11,1,11))
